#ifndef EVAL_OPERATION_H
#define EVAL_OPERATION_H
...
namespace op {
    ...
    public:
        ...
        Operation(uint8_t opc, tuple<string,uint8_t,uint8_t> instr):opcode(static_cast<Opcode>(opc)),mnemonic(get<0>(instr)),delta(get<1>(instr)),alpha(get<2>(instr)) { }
        ...
        virtual optional<instr::Instruction> toInstruction(stack<pair<unsigned,bitset<256>>>& stack, unsigned varCount) const;
    ...
    public:
        ...
        explicit Push(uint8_t opc, bitset<256> pV):Operation(opc), pushValue(pV){  }
        Push(const Push&) = delete;

        bitset<256> getPushValue() const override;

        optional<instr::Instruction> toInstruction(stack<pair<unsigned,bitset<256>>>& stack, unsigned varCount) const override;
    ...
    public:
        explicit Swap(uint8_t opc):Operation(opc) { }
    ...
    public:
        explicit Dup(uint8_t opc):Operation(opc) { }
